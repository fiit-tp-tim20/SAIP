...
class TestCustomer:
    def _generate_dict(
        self, products: List[Product], item_count: int = 0, instruction: str = ""
    ) -> dict:
        if instruction != "":
        ...
        return {"average": average, "products": _normal_set}

    def _generate_broken_dict(
        self, products: List[Product], item_count: int, instruction: str
    ) -> dict:
        _broken_set = {}
        for i in range(item_count):
            if i == item_count - 1:
        ...
        return {"average": instruction, "products": _broken_set}
    ...
    def setup_product_sets(self):
        ...
        product_sets = [
            self._generate_dict(products, item_count, instruction)
            for item_count, instruction in instructions
        ]
        yield product_sets

        print(
            "_________________________________________________TEARDOWN_________________________________________________"
        )
